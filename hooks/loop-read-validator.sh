...
# - Round files from wrong locations (not in .loop/rlcr/)
...
LOOP_BASE_DIR="${LOOP_BASE_DIR:-$PROJECT_ROOT/.loop/rlcr}"
...
IN_LOOP_LOOP_DIR=$(is_in_loop_loop_dir "$FILE_PATH" && echo "true" || echo "false")
if [[ "$IS_ROUND_FILE" != "true" ]] && ! { [[ "$IS_GOAL_TRACKER" == "true" ]] && [[ "$IN_LOOP_LOOP_DIR" == "true" ]]; }; then
    exit 0
fi
...
if [[ "$IS_GOAL_TRACKER" == "true" ]] && [[ "$IN_LOOP_LOOP_DIR" == "true" ]]; then
...
if [[ "$IN_LOOP_LOOP_DIR" == "false" ]]; then
