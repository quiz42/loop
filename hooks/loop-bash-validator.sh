...
# ========================================
# Block Git Add Commands Targeting .loop
# ========================================
# Prevents force-adding .loop files to version control
# Note: .loop is in .gitignore, but git add -f bypasses it

if git_adds_loop "$COMMAND_LOWER"; then
    git_add_loop_blocked_message >&2
    exit 2
fi
...
if command_modifies_file "$COMMAND_LOWER" "\.loop/rlcr(/[^/]+)?/plan\.md"; then
