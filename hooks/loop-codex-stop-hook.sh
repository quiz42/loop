...
#
# State directory: .loop/rlcr/<timestamp>/
...
BITLESSON_FILE_REL=".loop/bitlesson.md"
...
if [[ "${LOOP_ALLOW_EMPTY_BITLESSON_NONE:-}" == "true" ]]; then
    BITLESSON_ALLOW_EMPTY_NONE="true"
fi
...
if [[ -z "$RAW_FULL_REVIEW_ROUND" ]]; then
    ...
    echo "  To use configurable Full Alignment Check intervals, upgrade to loop v1.5.2+" >&2
...
# artifacts (.loop/rlcr/...) may make the working tree appear dirty
# if .loop is tracked, which would block exit before reaching this handler.
...
if [[ "$IS_METHODOLOGY_ANALYSIS_PHASE" == "true" ]]; then
    if complete_methodology_analysis; then
        ...
        #
        # Apply the same .loop/ untracked exclusion the main gate uses
        # so methodology-artifact writes under .loop/rlcr/... do not
        # themselves trip the check.
        if [[ "$GIT_IS_REPO" == "true" ]]; then
            LOOP_UNTRACKED_PATTERN='^\?\? \.loop[-/]'
            GIT_STATUS_FOR_BLOCK=$(echo "$GIT_STATUS_CACHED" | grep -vE "$LOOP_UNTRACKED_PATTERN" || true)
...
if [[ "$GIT_IS_REPO" == "true" ]]; then
    ...
    if git_has_tracked_loop_state "$PROJECT_ROOT"; then
        cleanup_stale_index_lock
        REASON=$(git_tracked_loop_blocked_message)

        jq -n \
            --arg reason "$REASON" \
            --arg msg "Loop: Blocked - tracked Loop state detected, remove it from git first" \
            '{
                "decision": "block",
                "reason": $reason,
                "systemMessage": $msg
    ...
    # Exclude untracked .loop/ paths and .loop-* dash-separated legacy
    # variants from the dirty determination because local plugin state under
    # .loop/ (.loop/bitlesson.md, config.json, rlcr/) is intentionally
    # untracked.
    LOOP_UNTRACKED_PATTERN='^\?\? \.loop[-/]'
    GIT_STATUS_FOR_BLOCK=$(echo "$GIT_STATUS_CACHED" | grep -vE "$LOOP_UNTRACKED_PATTERN" || true)
    if [[ -n "$GIT_STATUS_FOR_BLOCK" ]]; then
        ...
        # Check if .loop/ or .loop-* dash-separated legacy variants are untracked.
        if echo "$UNTRACKED" | grep -qE "$LOOP_UNTRACKED_PATTERN"; then
            LOOP_LOCAL_NOTE=$(load_template "$TEMPLATE_DIR" "block/git-not-clean-loop-local.md" 2>/dev/null)
            if [[ -z "$LOOP_LOCAL_NOTE" ]]; then
                LOOP_LOCAL_NOTE="Note: .loop/ and .loop-* directories are intentionally untracked."
            fi
            SPECIAL_NOTES="$SPECIAL_NOTES$LOOP_LOCAL_NOTE"
        fi

        # Check for other untracked files (potential artifacts)
        OTHER_UNTRACKED=$(echo "$UNTRACKED" | grep -vE "$LOOP_UNTRACKED_PATTERN" || true)
...
for (( r = CURRENT_ROUND - 1; r >= 0 && r >= CURRENT_ROUND - 3; r-- )); do
    RECENT_ROUND_FILES+="- @.loop/rlcr/${LOOP_TIMESTAMP}/round-${r}-summary.md
- @.loop/rlcr/${LOOP_TIMESTAMP}/round-${r}-review-result.md
"
done
...
# Debug log files go to XDG_CACHE_HOME/loop/<project-path>/<timestamp>/ to avoid polluting project dir
...
CACHE_DIR="$CACHE_BASE/loop/$SANITIZED_PROJECT_PATH/$LOOP_TIMESTAMP"
mkdir -p "$CACHE_DIR"
...
if [[ "${LOOP_CODEX_BYPASS_SANDBOX:-}" == "true" ]] || [[ "${LOOP_CODEX_BYPASS_SANDBOX:-}" == "1" ]]; then
...
Loop cannot safely update drift state or choose the correct next-round prompt without this verdict.
...
if [[ "$LAST_LINE_TRIMMED" == "$MARKER_STOP" ]]; then
    ...
    if [[ "$FULL_ALIGNMENT_CHECK" == "true" ]]; then
        ...
        echo "" >&2
        echo "Review the historical round files in .loop/rlcr/$(basename "$LOOP_DIR")/ to understand what went wrong." >&2
        echo "Consider:" >&2
