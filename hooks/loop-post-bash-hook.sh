...
#
# Mechanism:
# 1. Setup script creates .loop/.pending-session-id with:
...
SIGNAL_FILE="$PROJECT_ROOT/.loop/.pending-session-id"
...
if [[ -n "$COMMAND_SIGNATURE" ]]; then
    ...
    # Normalize consecutive slashes (e.g. "FrankDan77//scripts" -> "FrankDan77/scripts").
