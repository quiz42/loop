...
The `Mainline Progress Verdict` line is mandatory. If you omit it, the Loop stop hook will block the round and require the review to be rerun.
...
The project's `.loop/rlcr/{{LOOP_TIMESTAMP}}/` directory contains the history of each round's iteration:
...
- `@.loop/rlcr/{{LOOP_TIMESTAMP}}/round-{{PREV_ROUND}}-review-result.md` (previous round)
- `@.loop/rlcr/{{LOOP_TIMESTAMP}}/round-{{PREV_PREV_ROUND}}-review-result.md` (2 rounds ago)
- `@.loop/rlcr/{{LOOP_TIMESTAMP}}/round-{{PREV_ROUND}}-summary.md` (previous summary)
