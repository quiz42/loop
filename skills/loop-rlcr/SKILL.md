...
Codex installs of Loop require native hooks support and install the Loop `Stop` hooks automatically.
...
All commands below assume `{{LOOP_RUNTIME_ROOT}}`.
...
```bash
"{{LOOP_RUNTIME_ROOT}}/scripts/setup-rlcr-loop.sh" $ARGUMENTS
```
...
1. Read current loop prompt from `.loop/rlcr/<timestamp>/round-<N>-prompt.md` (or `finalize` prompt files when in finalize phase).
2. Implement required changes.
3. Commit changes.
4. Write required summary file:
   - Normal phase: `.loop/rlcr/<timestamp>/round-<N>-summary.md`
   - Finalize phase: `.loop/rlcr/<timestamp>/finalize-summary.md`
5. Stop or exit normally.
6. Let the native Loop `Stop` hook run automatically.
