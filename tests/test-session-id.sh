...
# Find the state file
STATE_FILE=$(find "$TEST_DIR/project/.loop/rlcr" -name "state.md" -type f 2>/dev/null | head -1)
...
SIGNAL_FILE="$TEST_DIR/project/.loop/.pending-session-id"
...
setup_test_dir
init_test_git_repo "$TEST_DIR/project"
mkdir -p "$TEST_DIR/project/.loop/rlcr/2026-01-01_00-00-00"
mkdir -p "$TEST_DIR/project/.loop"

# Create state.md with empty session_id
cat > "$TEST_DIR/project/.loop/rlcr/2026-01-01_00-00-00/state.md" << 'EOF'
---
current_round: 0
max_iterations: 10
codex_model: gpt-5.5
...
printf '%s\n%s\n' "$TEST_DIR/project/.loop/rlcr/2026-01-01_00-00-00/state.md" "$MOCK_SETUP_PATH" > "$TEST_DIR/project/.loop/.pending-session-id"
...
if [[ -f "$POST_HOOK" ]]; then
    ...
    RECORDED_ID=$(grep "^session_id:" "$TEST_DIR/project/.loop/rlcr/2026-01-01_00-00-00/state.md" | sed 's/session_id: *//')
    ...
    if [[ ! -f "$TEST_DIR/project/.loop/.pending-session-id" ]]; then
...
if [[ -f "$TEST_DIR/project/.loop/rlcr/2026-01-01_00-00-00/cancel-state.md" ]]; then
...
mkdir -p "$TEST_DIR/project/.loop/rlcr/2026-02-01_00-00-00"
cat > "$TEST_DIR/project/.loop/rlcr/2026-02-01_00-00-00/complete-state.md" << 'EOF'
---
current_round: 10
max_iterations: 10
...
if [[ -f "$TEST_DIR/project/.loop/rlcr/2026-01-01_00-00-00/state.md" ]]; then
