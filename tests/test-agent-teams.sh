...
mkdir -p "$TEST_DIR/project/.loop" "$TEST_DIR/project/temp"
cat > "$TEST_DIR/project/temp/plan.md" << 'EOF'
# Test Plan
...
printf '{"agent_teams": true}' > "$TEST_DIR/project/.loop/config.json"
echo "temp/" > "$TEST_DIR/project/.gitignore"
cd "$TEST_DIR/project"
git add .gitignore
git commit -q -m "Add gitignore"
...
PROMPT_FILE=$(find "$TEST_DIR/project/.loop/rlcr" -name "round-0-prompt.md" -type f 2>/dev/null | head -1)
