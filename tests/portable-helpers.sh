...
#
# This is the shell implementation from scripts/lib/shell-timeout.sh, which the
# production chain in scripts/portable-timeout.sh also ends in. It used to be a
# second copy here, back when that chain went gtimeout -> timeout -> python3 ->
# nothing and so could not be reused: ADR-0003 keeps the Bash layer off Python.
# Now there is one implementation, and every assertion the suites make about
# portable_run_with_timeout is an assertion about the code that times out
# `codex` in production.
#
# See scripts/lib/shell-timeout.sh for what is and is not guaranteed -- in
# particular the process-group and descendant-signalling rules, and the one
# case a shell cannot close (a descendant that leaves the tree after the
# snapshot).
#
...
_LOOP_PORTABLE_HELPERS_SELF="${BASH_SOURCE[0]:-$0}"
case "$_LOOP_PORTABLE_HELPERS_SELF" in
    */*) _LOOP_PORTABLE_HELPERS_DIR="${_LOOP_PORTABLE_HELPERS_SELF%/*}" ;;
    *) _LOOP_PORTABLE_HELPERS_DIR="." ;;
esac
source "$_LOOP_PORTABLE_HELPERS_DIR/../scripts/lib/shell-timeout.sh"
unset _LOOP_PORTABLE_HELPERS_SELF _LOOP_PORTABLE_HELPERS_DIR
