...
# Helper: create a mock .loop/bitlesson.md with required content
create_mock_bitlesson() {
    local dir="$1"
    mkdir -p "$dir/.loop"
    cat > "$dir/.loop/bitlesson.md" <<'EOF'
...
setup_test_dir
create_real_loop_bitlesson "$TEST_DIR"
BIN_DIR="$TEST_DIR/bin"
create_mock_codex "$BIN_DIR"
mkdir -p "$TEST_DIR/.loop"
printf '{"bitlesson_model": "gpt-4o"}' > "$TEST_DIR/.loop/config.json"
...
mkdir -p "$TEST_DIR/.loop"
printf '{"bitlesson_model": "haiku"}' > "$TEST_DIR/.loop/config.json"
...
create_mock_claude "$BIN_DIR"
mkdir -p "$TEST_DIR/.loop"
printf '{"bitlesson_model": "claude-3-5-sonnet-20241022"}' > "$TEST_DIR/.loop/config.json"
...
create_mock_claude "$BIN_DIR"
mkdir -p "$TEST_DIR/.loop"
printf '{"bitlesson_model": "claude-3-OPUS-20240229"}' > "$TEST_DIR/.loop/config.json"
...
create_real_loop_bitlesson "$TEST_DIR"
mkdir -p "$TEST_DIR/.loop"
printf '{"bitlesson_model": "unknown-xyz-model"}' > "$TEST_DIR/.loop/config.json"
...
mkdir -p "$TEST_DIR/.loop"
printf '{"bitlesson_model": "haiku", "codex_model": "gpt-5.5", "provider_mode": "codex-only"}' > "$TEST_DIR/.loop/config.json"
...
mkdir -p "$TEST_DIR/.loop"
printf '{"bitlesson_model": "gpt-5.5"}' > "$TEST_DIR/.loop/config.json"
