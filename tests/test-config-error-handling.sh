...
printf 'not valid json at all' > "$PROJECT_DIR/.loop/config.json"
...
mkdir -p "$PROJECT_DIR"
mkdir -p "$TEST_DIR/bad-user-cfg/loop"
printf '{bad json here}' > "$TEST_DIR/bad-user-cfg/loop/config.json"
...
mkdir -p "$PROJECT_DIR/.loop"
printf '{}' > "$PROJECT_DIR/.loop/config.json"
