#!/usr/bin/env bash
#
# Ensure Loop's nested Codex reviewer calls disable native hooks to avoid recursion.
#
...
# The stop hook resolves the hooks feature name via \`codex features list\`;
# advertise the modern \`hooks\` feature so it disables the correct one.
...
setup_loop_dir() {
    ...
    local loop_dir="$repo_dir/.loop/rlcr/2026-03-14_12-00-00"
