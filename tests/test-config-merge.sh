...
# - LOOP_CONFIG env var overrides the default project config path
#
...
printf '{"bitlesson_model": "opus"}' > "$PROJECT_DIR/.loop/config.json"
...
mkdir -p "$PROJECT_DIR/.loop"
mkdir -p "$TEST_DIR/user-cfg/loop"
printf '{"bitlesson_model": "user-model"}' > "$TEST_DIR/user-cfg/loop/config.json"
printf '{"bitlesson_model": "project-model"}' > "$PROJECT_DIR/.loop/config.json"
...
mkdir -p "$PROJECT_DIR/.loop"
mkdir -p "$TEST_DIR/user-cfg2/loop"
printf '{"bitlesson_model": "user-bitlesson"}' > "$TEST_DIR/user-cfg2/loop/config.json"
printf '{"gen_plan_mode": "project-plan-mode"}' > "$PROJECT_DIR/.loop/config.json"
...
mkdir -p "$PROJECT_DIR/.loop"
printf '{"bitlesson_model": null}' > "$PROJECT_DIR/.loop/config.json"
...
# ========================================
# Test 6: LOOP_CONFIG env var overrides default project config path
# ========================================

setup_test_dir
PROJECT_DIR="$TEST_DIR/custom-config-path"
mkdir -p "$PROJECT_DIR/.loop"
printf '{"bitlesson_model": "ignored-default-project"}' > "$PROJECT_DIR/.loop/config.json"

custom_cfg="$TEST_DIR/my-custom.json"
printf '{"bitlesson_model": "from-custom-path"}' > "$custom_cfg"

merged=$(XDG_CONFIG_HOME="$TEST_DIR/no-user-cfg4" LOOP_CONFIG="$custom_cfg" \
    load_merged_config "$PROJECT_ROOT" "$PROJECT_DIR" 2>/dev/null)

val=$(get_config_value "$merged" "bitlesson_model")
if [[ "$val" == "from-custom-path" ]]; then
    pass "LOOP_CONFIG: custom config path via env var overrides project default"
else
    fail "LOOP_CONFIG: custom config path via env var overrides project default" \
...
mkdir -p "$PROJECT_DIR/.loop"
mkdir -p "$TEST_DIR/user-cfg-all/loop"
printf '{"gen_plan_mode": "user-plan-mode"}' > "$TEST_DIR/user-cfg-all/loop/config.json"
printf '{"agent_teams": true}' > "$PROJECT_DIR/.loop/config.json"
