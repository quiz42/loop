...
SKILL_DIRS_BEFORE=$(find "$MOCK_PROJECT/.loop/skill" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | sort)
reset_mock
export MOCK_CODEX_STDOUT="Test output for file"
run_ask_codex "file test" > /dev/null 2>&1
SKILL_DIRS_AFTER=$(find "$MOCK_PROJECT/.loop/skill" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | sort)
...
LATEST_DIR=$(find "$MOCK_PROJECT/.loop/skill" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | sort | tail -1)
...
DIRS_BEFORE=$(find "$MOCK_PROJECT/.loop/skill" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | sort)
...
DIRS_AFTER=$(find "$MOCK_PROJECT/.loop/skill" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | sort)
...
CACHE_BASE="$TEST_DIR/cache/loop"
