...
# Source loop.sh to get production functions
...
RESULT=$(loop_parse_goal_tracker "$TEST_DIR/goal-tracker.md" 2>/dev/null || echo "error")
...
# Call production function directly (this is what monitor calls internally)
RESULT=$(loop_parse_git_status 2>/dev/null || echo "error")
...
# Verify loop_detect_git_state is callable
STATE=$(loop_detect_git_state 2>/dev/null || echo "error")
...
# ========================================
# These tests invoke the real _loop_monitor_codex function
...
# Create project with NO .loop directory
...
mkdir -p "$MONITOR_TEST_DIR/project2/.loop/rlcr/2026-01-17_10-00-00"
cat > "$MONITOR_TEST_DIR/project2/.loop/rlcr/2026-01-17_10-00-00/state.md" << 'STATE_EOF'
---
current_round: 1
max_iterations: 5
---
STATE_EOF
cat > "$MONITOR_TEST_DIR/project2/.loop/rlcr/2026-01-17_10-00-00/goal-tracker.md" << 'GOAL_EOF'
# Goal Tracker
## IMMUTABLE SECTION
### Ultimate Goal
...
mkdir -p "$FAKE_HOME_MONITOR/.cache/loop/$SANITIZED/2026-01-17_10-00-00"
echo "Test log" > "$FAKE_HOME_MONITOR/.cache/loop/$SANITIZED/2026-01-17_10-00-00/round-1-codex-run.log"

# Create narrow terminal runner - calls _loop_monitor_codex directly in same shell
...
(sleep 1 && rm -rf "$PROJECT_DIR/.loop/rlcr") &
DELETE_PID=$!
...
mkdir -p "$MONITOR_TEST_DIR/project3/.loop/rlcr/2026-01-17_11-00-00"
cat > "$MONITOR_TEST_DIR/project3/.loop/rlcr/2026-01-17_11-00-00/state.md" << 'STATE_EOF'
---
current_round: 1
max_iterations: 5
---
STATE_EOF
cat > "$MONITOR_TEST_DIR/project3/.loop/rlcr/2026-01-17_11-00-00/goal-tracker.md" << 'GOAL_EOF'
# Goal Tracker
## IMMUTABLE SECTION
### Ultimate Goal
Test ANSI codes.
...
mkdir -p "$FAKE_HOME_MONITOR/.cache/loop/$SANITIZED3/2026-01-17_11-00-00"
# Write log with ANSI codes
printf '\033[31mRed text\033[0m\n\033[1;32mBold green\033[0m\n' > "$FAKE_HOME_MONITOR/.cache/loop/$SANITIZED3/2026-01-17_11-00-00/round-1-codex-run.log"
