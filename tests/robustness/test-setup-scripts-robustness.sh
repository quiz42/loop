...
# Test 16b: Untracked .loopconfig still blocks setup as a dirty working tree
echo ""
echo "Test 16b: Untracked .loopconfig is not ignored as runtime state"
...
touch "$TEST_DIR/repo16b/.loopconfig"
...
mkdir -p "$TEST_DIR/repo24/.loop/rlcr/2026-01-19_00-00-00"
cat > "$TEST_DIR/repo24/.loop/rlcr/2026-01-19_00-00-00/state.md" << 'EOF'
---
current_round: 0
max_iterations: 42
---
...
LOOP_DIR=$(find "$TEST_DIR/repo42/.loop/rlcr" -maxdepth 1 -type d -name "20*" 2>/dev/null | head -1)
...
LOOP_DIR_45=$(find "$TEST_DIR/repo45/.loop/rlcr" -maxdepth 1 -type d -name "20*" 2>/dev/null | head -1)
