...
setup_active_loop_fixture() {
    ...
    mkdir -p "$project_dir/.loop/rlcr/2026-03-01_00-00-00"
...
PLANEOF
    ...
    cp "$project_dir/plan.md" "$project_dir/.loop/rlcr/2026-03-01_00-00-00/plan.md"

    cat > "$project_dir/.loop/rlcr/2026-03-01_00-00-00/state.md" <<EOF_STATE
---
current_round: 0
max_iterations: 42
codex_model: gpt-5.5
...
# Test 3: Tracked Loop state blocks before normal loop validation
T3_DIR="$TEST_DIR/t3"
mkdir -p "$T3_DIR"
setup_active_loop_fixture "$T3_DIR/project"
echo "tracked" > "$T3_DIR/project/.loop/rlcr/2026-03-01_00-00-00/goal-tracker.md"
git -C "$T3_DIR/project" add -f .loop/rlcr/2026-03-01_00-00-00/goal-tracker.md
...
if [[ "$EXIT3" -eq 10 ]]; then
    pass "rlcr-stop-gate blocks tracked Loop state"
else
    OUTPUT3=$(cat "$T3_DIR/out.txt" 2>/dev/null || true)
    fail "rlcr-stop-gate blocks tracked Loop state" "exit 10" "exit $EXIT3; output: $OUTPUT3"
fi

if grep -q "Tracked Loop State Blocked" "$T3_DIR/out.txt" 2>/dev/null; then
    pass "rlcr-stop-gate reports tracked Loop state with dedicated reason"
else
    OUTPUT3=$(cat "$T3_DIR/out.txt" 2>/dev/null || true)
    fail "rlcr-stop-gate reports tracked Loop state with dedicated reason" "output containing Tracked Loop State Blocked" "$OUTPUT3"
fi

# Test 4: Unrelated dot-prefixed files that happen to start with .loop-
# must not be treated as loop state. .loop-backup and .loopconfig are
# explicitly allowed by the git add validator (tests/test-loop-escape.sh);
# the tracked-state guard must stay consistent and ignore them.
T4_DIR="$TEST_DIR/t4"
mkdir -p "$T4_DIR"
setup_active_loop_fixture "$T4_DIR/project"
echo "not loop state" > "$T4_DIR/project/.loop-backup"
echo "not loop state" > "$T4_DIR/project/.loopconfig"
git -C "$T4_DIR/project" add -f .loop-backup .loopconfig
...
if [[ "$EXIT4" -eq 10 ]]; then
    pass "rlcr-stop-gate does not confuse .loop-backup with loop state"
else
    OUTPUT4=$(cat "$T4_DIR/out.txt" 2>/dev/null || true)
    fail "rlcr-stop-gate does not confuse .loop-backup with loop state" "exit 10" "exit $EXIT4; output: $OUTPUT4"
fi

if ! grep -q "Tracked Loop State Blocked" "$T4_DIR/out.txt" 2>/dev/null; then
    pass "rlcr-stop-gate does not emit tracked-state reason for .loop-backup"
else
    OUTPUT4=$(cat "$T4_DIR/out.txt" 2>/dev/null || true)
    fail "rlcr-stop-gate does not emit tracked-state reason for .loop-backup" "no Tracked Loop State Blocked line" "$OUTPUT4"
fi
...
# Layout expected by rlcr-stop-gate.sh: LOOP_ROOT/hooks/loop-codex-stop-hook.sh.
