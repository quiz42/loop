#!/usr/bin/env bash
#
# Tests for _loop_monitor_skill (loop monitor skill)
#
...
setup_test_env() {
    ...
    # Source loop.sh (which sources monitor-common.sh and monitor-skill.sh)
...
create_skill_invocation() {
    ...
    local dir=".loop/skill/$unique_id"
    mkdir -p "$dir"
...
setup_test_env
output=$(_loop_monitor_skill --once 2>&1) && rc=0 || rc=$?
if [[ $rc -ne 0 ]] && grep -q "directory not found" <<< "$output"; then
    pass "Returns error when .loop/skill does not exist"
else
...
local_dir=".loop/skill/2026-02-19_22-00-00-555-eee"
mkdir -p "$local_dir"
...
mkdir -p ".loop/skill/not-a-skill-dir"
echo "junk" > ".loop/skill/not-a-skill-dir/input.md"
