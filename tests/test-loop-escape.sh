#!/usr/bin/env bash
#
# Test script for loop-escape fixes
#
# Tests:
# 1. Zsh safety for empty/dotfile directory scenarios
# 2. git_adds_loop path variant detection (./.loop, quoted paths)
#
# These tests verify the fixes for:
# - No zsh/bash "no matches found" errors
# - Block git add .loop (including path variants)
#
...
# ========================================
# Test Group 1: git_adds_loop Path Variants
# ========================================
...
assert_blocks() {
    ...
    if git_adds_loop "$command_lower"; then
        pass "$description"
    else
...
echo "========================================"
echo "Testing loop-escape Fixes"
echo "========================================"
echo ""

# ========================================
# Test Group 1: ./.loop Path Variants
# ========================================
echo "Test Group 1: ./.loop Path Variants"
echo ""

assert_blocks "git add ./.loop" "Block: ./.loop prefix"
assert_blocks "git add ./.loop/" "Block: ./.loop/ with trailing slash"
assert_blocks "git add ./.loop/file.md" "Block: ./.loop/file.md"
assert_blocks "git add path/to/.loop" "Block: path/to/.loop"
assert_blocks "git add ../project/.loop" "Block: ../project/.loop"
assert_blocks "git add .loop/rlcr/2026-03-01_00-00-00/goal-tracker.md" "Block: RLCR goal tracker path"
assert_blocks "git add .loop/rlcr/2026-03-01_00-00-00/round-3-summary.md" "Block: RLCR round summary path"
assert_blocks "git add .loop/rlcr/2026-03-01_00-00-00/round-3-contract.md" "Block: RLCR round contract path"
...
assert_blocks 'git add ".loop"' "Block: double-quoted .loop"
assert_blocks "git add '.loop'" "Block: single-quoted .loop"
assert_blocks 'git add "./.loop"' "Block: double-quoted ./.loop"
assert_blocks "git add './.loop'" "Block: single-quoted ./.loop"
assert_blocks 'git add "path/to/.loop"' "Block: double-quoted path/to/.loop"
assert_blocks 'git add ".loop/rlcr/2026-03-01_00-00-00/goal-tracker.md"' "Block: double-quoted RLCR file path"
...
assert_blocks "git add -f ./.loop" "Block: -f with ./.loop"
assert_blocks "git add --force ./.loop" "Block: --force with ./.loop"
assert_blocks 'git add -f ".loop"' "Block: -f with quoted .loop"
assert_blocks "git add -f .loop/rlcr/2026-03-01_00-00-00/goal-tracker.md" "Block: -f with RLCR goal tracker"
...
# These tests require .loop directory to exist for blocking to trigger
# (git_adds_loop only blocks -A/--all when .loop exists)
TEST_LOOP_DIR="/tmp/test-loop-git-add-$$"
mkdir -p "$TEST_LOOP_DIR/.loop"
ORIGINAL_DIR="$(pwd)"
cd "$TEST_LOOP_DIR"

assert_blocks "git add -A" "Block: -A (adds all including .loop)"
assert_blocks "git add --all" "Block: --all (adds all including .loop)"
...
assert_blocks "cd repo && git add ./.loop" "Block: cd && git add ./.loop"
assert_blocks "true; git add ./.loop" "Block: true; git add ./.loop"
assert_blocks 'echo test && git add ".loop"' "Block: echo && git add quoted"
...
assert_blocks "git -C /path add ./.loop" "Block: git -C with ./.loop"
assert_blocks 'git -C /path add ".loop"' "Block: git -C with quoted .loop"
assert_blocks "git --git-dir=/repo add ./.loop" "Block: --git-dir with ./.loop"
...
assert_allows "git add src/.gitkeep" "Allow: .gitkeep (not .loop)"
assert_allows "git add .gitignore" "Allow: .gitignore"
assert_allows "git add ./src/" "Allow: ./src/ directory"
assert_allows "git status .loop" "Allow: git status (not add)"
assert_allows "git diff .loop" "Allow: git diff (not add)"
assert_allows "git log -- .loop" "Allow: git log (not add)"
...
# Files that start with .loop but are NOT the .loop directory
assert_allows "git add .loopconfig" "Allow: .loopconfig (different file)"
assert_allows "git add .loop-backup" "Allow: .loop-backup (different file)"
assert_allows "git add src/.looprc" "Allow: src/.looprc (different file)"
...
test_empty_dir() {
    local test_dir="/tmp/test-loop-empty-$$"
    mkdir -p "$test_dir"

    # Simulate the find-based iteration pattern used in loop.sh
    local found_count=0
...
test_dotfiles_only_dir() {
    local test_dir="/tmp/test-loop-dotfiles-$$"
    mkdir -p "$test_dir"
...
test_no_state_md_files() {
    local test_dir="/tmp/test-loop-nostate-$$"
    mkdir -p "$test_dir"
...
test_state_md_detection() {
    local test_dir="/tmp/test-loop-state-$$"
    mkdir -p "$test_dir"
...
test_session_dir_detection() {
    local test_dir="/tmp/test-loop-sessions-$$"
    mkdir -p "$test_dir"
