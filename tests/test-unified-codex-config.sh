...
# run_with_timeout: GNU coreutils is not on the macOS GitHub runner, so a
# bare "timeout" would exit 127 there. Same wrapper the robustness suites use.
...
if [[ ! -f "$CONFIG_LOADER" ]]; then
    ...
    printf '{"codex_model": "gpt-5.2", "codex_effort": "xhigh"}' > "$PROJECT_DIR/.loop/config.json"
...
if [[ ! -f "$LOOP_COMMON" ]]; then
    ...
    mkdir -p "$OVERRIDE_PROJECT/.loop"
    printf '{"codex_model": "o3-mini", "codex_effort": "low"}' > "$OVERRIDE_PROJECT/.loop/config.json"
    ...
    mkdir -p "$INVALID_PROJECT/.loop"
    printf '{"codex_model": "haiku!", "codex_effort": "superhigh"}' > "$INVALID_PROJECT/.loop/config.json"
    ...
    for invalid_model in haiku false claude-3; do
        ...
        mkdir -p "$INVALID_PROJECT/.loop"
        printf '{"codex_model": "%s"}' "$invalid_model" > "$INVALID_PROJECT/.loop/config.json"
...
BARE_EOF
    ...
    mkdir -p "$OVERRIDE_PROJECT/.loop"
    printf '{"codex_model": "o1-preview", "codex_effort": "medium"}' > "$OVERRIDE_PROJECT/.loop/config.json"
...
if [[ ! -f "$LOOP_COMMON" ]]; then
    ...
    mkdir -p "$STALE_PROJECT/.loop"
    printf '{"loop_reviewer_model": "o3-mini", "loop_reviewer_effort": "low", "codex_model": "gpt-5.3"}' > "$STALE_PROJECT/.loop/config.json"
...
if [[ ! -f "$STOP_HOOK" ]]; then
    ...
    mkdir -p "$HOOK_PROJECT/.loop/rlcr/2099-01-01_00-00-00"

    # Create state.md with invalid codex effort
    cat > "$HOOK_PROJECT/.loop/rlcr/2099-01-01_00-00-00/state.md" << 'HOOK_STATE_EOF'
---
current_round: 1
max_iterations: 10
codex_model: gpt-5.5
...
if ! command -v jq >/dev/null 2>&1; then
    ...
    mkdir -p "$EXEC_PROJECT/.loop"
    printf '{"codex_model": "gpt-5.2", "codex_effort": "low"}' > "$EXEC_PROJECT/.loop/config.json"
...
PLAN_EOF
    ...
    STATE_FILE=$(find "$EXEC_PROJECT/.loop/rlcr" -name "state.md" 2>/dev/null | head -1 || true)
...
if [[ ! -f "$ASK_CODEX" ]]; then
    ...
    mkdir -p "$ASK_CFG_PROJECT/.loop"
    printf '{"codex_model": "o3-mini", "codex_effort": "low"}' > "$ASK_CFG_PROJECT/.loop/config.json"
