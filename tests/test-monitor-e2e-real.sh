...
#
# This test runs the REAL _loop_monitor_codex function (not stubs)
# and verifies graceful stop behavior when .loop/rlcr is deleted.
#
# Validates:
# - Clean exit with user-friendly message when .loop deleted
...
monitor_test_bash_deletion() {
    echo "Test 1: Real _loop_monitor_codex with directory deletion (bash)"
    echo ""

    # Create test project directory
    TEST_PROJECT="$TEST_BASE/project1"
    mkdir -p "$TEST_PROJECT/.loop/rlcr/2026-01-16_10-00-00"

    # Create valid state.md file
    cat > "$TEST_PROJECT/.loop/rlcr/2026-01-16_10-00-00/state.md" << 'STATE'
---
current_round: 1
max_iterations: 5
codex_model: o3
...
STATE
    ...
    cat > "$TEST_PROJECT/.loop/rlcr/2026-01-16_10-00-00/goal-tracker.md" << 'GOALTRACKER_EOF1'
# Goal Tracker
## IMMUTABLE SECTION
### Ultimate Goal
Test goal
...
GOALTRACKER_EOF1
    ...
    CACHE_DIR="$FAKE_HOME/.cache/loop/$SANITIZED_PROJECT/2026-01-16_10-00-00"
    mkdir -p "$CACHE_DIR"
    echo "Round 1 started" > "$CACHE_DIR/round-1-codex-run.log"

    # Create the test runner script
    # This script runs the REAL _loop_monitor_codex function
    cat > "$TEST_PROJECT/run_real_monitor.sh" << 'MONITOR_SCRIPT'
#!/usr/bin/env bash
# Run the REAL _loop_monitor_codex function
...
# Source the loop.sh script to get the REAL _loop_monitor_codex function
source "$PROJECT_ROOT/scripts/loop.sh"

# Run the REAL monitor function and capture all output
_loop_monitor_codex 2>&1
exit_code=$?
...
MONITOR_SCRIPT
    ...
    if grep -q 'printf "\\033\[r"' "$PROJECT_ROOT/scripts/loop.sh"; then
...
# ========================================
# Test 2: Real _loop_monitor_codex with directory deletion (zsh)
# ========================================
monitor_test_zsh_deletion() {
    echo ""
    echo "Test 2: Real _loop_monitor_codex with directory deletion (zsh)"
    echo ""
    ...
    if ! command -v zsh &>/dev/null; then
        ...
        mkdir -p "$TEST_PROJECT_ZSH/.loop/rlcr/2026-01-16_11-00-00"

        # Create valid state.md file
        cat > "$TEST_PROJECT_ZSH/.loop/rlcr/2026-01-16_11-00-00/state.md" << 'STATE'
---
current_round: 1
max_iterations: 5
codex_model: o3
...
STATE
        ...
        cat > "$TEST_PROJECT_ZSH/.loop/rlcr/2026-01-16_11-00-00/goal-tracker.md" << 'GOALTRACKER_EOF'
# Goal Tracker
## IMMUTABLE SECTION
### Ultimate Goal
Test goal
...
GOALTRACKER_EOF
        ...
        CACHE_DIR_ZSH="$FAKE_HOME_ZSH/.cache/loop/$SANITIZED_PROJECT_ZSH/2026-01-16_11-00-00"
...
#!/bin/zsh
# Run the REAL _loop_monitor_codex function under zsh
...
# ========================================
# Test 3: Real _loop_monitor_codex with SIGINT/Ctrl+C
# ========================================
monitor_test_bash_sigint() {
    echo ""
    echo "Test 3: Real _loop_monitor_codex with SIGINT/Ctrl+C"
    echo ""

    # Create test project directory for SIGINT test
    TEST_PROJECT_SIGINT="$TEST_BASE/project_sigint"
    mkdir -p "$TEST_PROJECT_SIGINT/.loop/rlcr/2026-01-16_12-00-00"

    # Create valid state.md file
    cat > "$TEST_PROJECT_SIGINT/.loop/rlcr/2026-01-16_12-00-00/state.md" << 'STATE'
---
current_round: 1
max_iterations: 5
codex_model: o3
...
STATE
    ...
    # Create goal-tracker.md
    cat > "$TEST_PROJECT_SIGINT/.loop/rlcr/2026-01-16_12-00-00/goal-tracker.md" << 'GOALTRACKER_SIGINT'
# Goal Tracker
## IMMUTABLE SECTION
### Ultimate Goal
...
GOALTRACKER_SIGINT
    ...
    CACHE_DIR_SIGINT="$FAKE_HOME_SIGINT/.cache/loop/$SANITIZED_PROJECT_SIGINT/2026-01-16_12-00-00"
...
#!/usr/bin/env bash
# Run the REAL _loop_monitor_codex function for SIGINT testing
...
# ========================================
# Test 4: Real _loop_monitor_codex with SIGINT/Ctrl+C
# ========================================
monitor_test_zsh_sigint() {
    echo ""
    echo "Test 4: Real _loop_monitor_codex with SIGINT/Ctrl+C"
    echo ""
    ...
    if ! command -v zsh &>/dev/null; then
        ...
        mkdir -p "$TEST_PROJECT_ZSH_SIGINT/.loop/rlcr/2026-01-16_13-00-00"

        # Create state.md
        cat > "$TEST_PROJECT_ZSH_SIGINT/.loop/rlcr/2026-01-16_13-00-00/state.md" << 'STATE'
---
current_round: 1
max_iterations: 5
codex_model: o3
...
STATE
        ...
        # Create goal-tracker.md
        cat > "$TEST_PROJECT_ZSH_SIGINT/.loop/rlcr/2026-01-16_13-00-00/goal-tracker.md" << 'GOALTRACKER_ZSH_SIGINT'
# Goal Tracker
## IMMUTABLE SECTION
### Ultimate Goal
...
GOALTRACKER_ZSH_SIGINT
        ...
        CACHE_DIR_ZSH_SIGINT="$FAKE_HOME_ZSH_SIGINT/.cache/loop/$SANITIZED_PROJECT_ZSH_SIGINT/2026-01-16_13-00-00"
...
#!/bin/zsh
# Run the REAL _loop_monitor_codex function under zsh for SIGINT testing
