...
# Test 8a: Bash validator blocks direct .loop/rlcr/plan.md (no intermediate dir)
# This tests Fix #1 for the regex bypass vulnerability
echo "Test 8a: Block bash modifications to direct .loop/rlcr/plan.md"
HOOK_INPUT='{"tool_name": "Bash", "tool_input": {"command": "echo evil > .loop/rlcr/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-bash-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
...
HOOK_INPUT='{"tool_name": "Bash", "tool_input": {"command": "echo test > .loop/rlcr/$(date +%Y)/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-bash-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
...
HOOK_INPUT='{"tool_name": "Bash", "tool_input": {"command": "echo test > .loop/rlcr/*/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-bash-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
...
HOOK_INPUT='{"tool_name": "Bash", "tool_input": {"command": "tee .loop/rlcr/{a,b,c}/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-bash-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
...
HOOK_INPUT='{"tool_name": "Bash", "tool_input": {"command": "cat input.txt | tee .loop/rlcr/2024-01-01_12-00-00/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-bash-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
...
HOOK_INPUT='{"tool_name": "Bash", "tool_input": {"command": "echo test > .loop/rlcr/`echo test`/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-bash-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
...
TRACKED_LOOP_DIR="$PWD/.loop/rlcr/2024-01-01_12-00-00"
...
git config user.name "Test"
echo "init" > init.txt
# Add .loop to gitignore so it doesn't trigger uncommitted changes
echo ".loop*" > .gitignore
...
git -c commit.gpgsign=false commit -q -m "Add gitignore"
# Create loop directory
LOOP_DIR_14_1="$PWD/.loop/rlcr/2024-01-01_12-00-00"
...
git add .gitignore
git -c commit.gpgsign=false commit -q -m "Add gitignore"
LOOP_DIR_14_2="$PWD/.loop/rlcr/2024-01-01_12-00-00"
...
git add .gitignore
git -c commit.gpgsign=false commit -q -m "Add gitignore"
LOOP_DIR_14_3="$PWD/.loop/rlcr/2024-01-01_12-00-00"
...
git add .gitignore
git -c commit.gpgsign=false commit -q -m "Add gitignore"
LOOP_DIR_14_4="$PWD/.loop/rlcr/2024-01-01_12-00-00"
...
# Test 15: Bash validator ALLOWS writes to legacy .loop-loop.local (it's not a loop dir anymore)
echo "Test 15: Bash validator allows writes to legacy .loop-loop.local"
HOOK_INPUT='{"tool_name": "Bash", "tool_input": {"command": "echo test > .loop-loop.local/2024-01-01/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-bash-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
...
if [[ $EXIT_CODE -eq 0 ]]; then
    pass "Bash validator allows writes to legacy .loop-loop.local"
else
    fail "Bash validator legacy path" "exit 0 (allowed)" "exit $EXIT_CODE, output: $RESULT"
fi

# Test 16: Write validator ALLOWS writes to legacy .loop-loop.local plan.md
echo "Test 16: Write validator allows writes to legacy .loop-loop.local plan.md"
HOOK_INPUT='{"tool_name": "Write", "tool_input": {"file_path": "'$TEST_DIR'/.loop-loop.local/2024-01-01/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-write-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
if [[ $EXIT_CODE -eq 0 ]]; then
    pass "Write validator allows writes to legacy .loop-loop.local plan.md"
else
    fail "Write validator legacy path" "exit 0 (allowed)" "exit $EXIT_CODE, output: $RESULT"
fi

# Test 17: Edit validator ALLOWS edits to legacy .loop-loop.local plan.md
echo "Test 17: Edit validator allows edits to legacy .loop-loop.local plan.md"
HOOK_INPUT='{"tool_name": "Edit", "tool_input": {"file_path": "'$TEST_DIR'/.loop-loop.local/2024-01-01/plan.md"}}'
set +e
RESULT=$(echo "$HOOK_INPUT" | "$PROJECT_ROOT/hooks/loop-edit-validator.sh" 2>&1)
EXIT_CODE=$?
set -e
if [[ $EXIT_CODE -eq 0 ]]; then
    pass "Edit validator allows edits to legacy .loop-loop.local plan.md"
else
