...
LOOP_USER_CONFIG="$XDG_CONFIG_HOME_DIR/loop/config.json"
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "/tmp/old/skills/loop/hooks/loop-codex-stop-hook.sh",
            "timeout": 30
          }
        ]
      },
...
if [[ -f "$CODEX_HOME_DIR/skills/loop/SKILL.md" ]]; then
    pass "Codex install syncs Loop skill bundle"
else
    fail "Codex install syncs Loop skill bundle" "skills/loop/SKILL.md exists" "missing"
fi

if [[ -f "$CODEX_HOME_DIR/skills/loop-rlcr/SKILL.md" ]]; then
    pass "Codex install keeps loop-rlcr entrypoint skill"
else
    fail "Codex install keeps loop-rlcr entrypoint skill" "skills/loop-rlcr/SKILL.md exists" "missing"
fi
...
if [[ -f "$LOOP_USER_CONFIG" ]]; then
    pass "Codex install writes Loop user config"
else
    fail "Codex install writes Loop user config" "$LOOP_USER_CONFIG exists" "missing"
fi
...
if [[ "$(jq -r '.bitlesson_model // empty' "$LOOP_USER_CONFIG")" == "gpt-5.5" ]]; then
    ...
    fail "Codex install seeds bitlesson_model with a Codex/OpenAI model" \
        "gpt-5.5" "$(jq -c '.' "$LOOP_USER_CONFIG" 2>/dev/null || echo MISSING)"
fi

if [[ "$(jq -r '.provider_mode // empty' "$LOOP_USER_CONFIG")" == "codex-only" ]]; then
    pass "Codex install marks Loop user config as codex-only"
else
    fail "Codex install marks Loop user config as codex-only" \
        "codex-only" "$(jq -c '.' "$LOOP_USER_CONFIG" 2>/dev/null || echo MISSING)"
fi

runtime_root="$CODEX_HOME_DIR/skills/loop"
PY_OUTPUT="$(
...
print("OLD=" + ("1" if any("/tmp/old/skills/loop/hooks/" in cmd for cmd in commands) else "0"))
print("SESSION=" + ("1" if data["hooks"]["SessionStart"][0]["hooks"][0]["command"] == "/custom/session-start.sh" else "0"))
print("COUNT=" + str(sum(1 for cmd in commands if "/loop/hooks/" in cmd)))
PY
)"

if grep -q '^FOUND=1$' <<<"$PY_OUTPUT"; then
    pass "Codex install adds managed Loop Stop hook commands"
else
    fail "Codex install adds managed Loop Stop hook commands" "FOUND=1" "$PY_OUTPUT"
fi
...
if grep -q '^OLD=0$' <<<"$PY_OUTPUT"; then
    pass "Codex install removes stale Loop hook commands"
else
    fail "Codex install removes stale Loop hook commands" "OLD=0" "$PY_OUTPUT"
fi
...
if grep -q '^COUNT=1$' <<<"$PY_OUTPUT"; then
    pass "Codex install writes exactly one managed Loop Stop hook"
else
    fail "Codex install writes exactly one managed Loop Stop hook" "COUNT=1" "$PY_OUTPUT"
fi
...
print(sum(1 for cmd in commands if "/loop/hooks/" in cmd))
PY
)"
