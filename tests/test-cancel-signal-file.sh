...
setup_test_loop() {
    local test_name="$1"
    rm -rf "$TEST_DIR/.loop" 2>/dev/null || true
...
touch "$TEST_DIR/.loop/rlcr/.cancel-requested"
...
SYMLINK_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/loop-symlink-XXXXXXXX" | tr '[:upper:]' '[:lower:]')
...
[[ -d "$SYMLINK_ROOT" ]] || { rm -rf "$SYMLINK_ROOT" 2>/dev/null; SYMLINK_ROOT="${TMPDIR:-/tmp}/loop-symlink-lowercase-$$"; mkdir -p "$SYMLINK_ROOT"; }
