...
- Requirement 3
EOF
        # Add .loop, bin, and .cache to gitignore (they are created by tests)
...
# Cache path: $XDG_CACHE_HOME/loop/$SANITIZED_PROJECT_PATH/$LOOP_TIMESTAMP/round-N-codex-review.log
LOOP_TIMESTAMP=$(basename "$LOOP_DIR")
SANITIZED_PROJECT_PATH=$(echo "$TEST_DIR" | sed 's/[^a-zA-Z0-9._-]/-/g' | sed 's/--*/-/g')
REVIEW_CACHE_DIR="$XDG_CACHE_HOME/loop/$SANITIZED_PROJECT_PATH/$LOOP_TIMESTAMP"
