...
#
# Covers:
# - Untracked legacy .loop-* directories do not trigger git-dirty blocks
# - Untracked .loopconfig still triggers git-dirty blocks as a real file
# - Legacy loops without bitlesson_required stay disabled even if
#   .loop/bitlesson.md exists
#
...
EOF
    ...
    loop_dir="$repo_dir/.loop/rlcr/2026-03-01_00-00-00"
    mkdir -p "$loop_dir"
...
EOF
    ...
    if [[ "$include_bitlesson" == "true" ]]; then
        mkdir -p "$repo_dir/.loop"
        cat > "$repo_dir/.loop/bitlesson.md" << 'EOF'
# BitLesson
...
echo "Test 1: Untracked legacy .loop-* paths are ignored for dirty checks"
TEST1_REPO="$TEST_DIR/test1"
create_stop_hook_fixture "$TEST1_REPO"
mkdir -p "$TEST1_REPO/.loop-old"
echo "legacy state" > "$TEST1_REPO/.loop-old/legacy.txt"
...
if [[ "$RUN_EXIT_CODE" -eq 0 ]] && [[ -f "$RUN_MARKER" ]] && \
   ! echo "$RUN_OUTPUT" | grep -q "Loop: Blocked - uncommitted changes"; then
    ...
    fail \
        "Stop hook ignores untracked .loop-old paths when checking git dirtiness" \
...
echo "Test 1b: Untracked .loopconfig still blocks dirty checks"
TEST1B_REPO="$TEST_DIR/test1b"
create_stop_hook_fixture "$TEST1B_REPO"
touch "$TEST1B_REPO/.loopconfig"
# Also create a .loop-old directory to trigger the "Special Case" note.
# The .loop/ directory itself may be covered by a global gitignore
# so it might not appear as untracked; .loop-old/ is never globally ignored.
mkdir -p "$TEST1B_REPO/.loop-old"
echo "legacy" > "$TEST1B_REPO/.loop-old/legacy.txt"
...
if [[ "$RUN_EXIT_CODE" -eq 0 ]] && [[ ! -f "$RUN_MARKER" ]] && \
   ...
   echo "$RUN_OUTPUT" | grep -q "Note on Untracked Files"; then
    ...
    fail \
        "Stop hook treats .loopconfig as a normal untracked file" \
        "blocked before Codex with both loop-runtime and generic untracked-file notes" \
        "exit $RUN_EXIT_CODE, marker=$(test -f "$RUN_MARKER" && echo present || echo missing), output: $RUN_OUTPUT"
fi
...
if [[ "$RUN_EXIT_CODE" -eq 0 ]] && [[ -f "$RUN_MARKER" ]]; then
    pass "Legacy loops without bitlesson_required still reach Codex even when .loop/bitlesson.md exists"
else
    fail \
        "Legacy loops without bitlesson_required still reach Codex even when .loop/bitlesson.md exists" \
