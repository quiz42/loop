...
PROMPT_FILE=$(find "$TEST_DIR/repo-routing/.loop/rlcr" -name "round-0-prompt.md" -type f | head -1)
GOAL_TRACKER_FILE=$(find "$TEST_DIR/repo-routing/.loop/rlcr" -name "goal-tracker.md" -type f | head -1)
...
EOF
    ...
    local loop_dir="$repo_dir/.loop/rlcr/2024-02-01_12-00-00"
    mkdir -p "$loop_dir"
...
NEXT_PROMPT="$TEST_DIR/hook-routing/.loop/rlcr/2024-02-01_12-00-00/round-1-prompt.md"
