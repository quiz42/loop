...
Detailed usage documentation for the Loop plugin. For installation, see [Install for Claude Code](install-for-claude.md).

## How It Works

Loop creates an iterative feedback loop with two phases:
...
Before the RLCR loop starts any work, Loop runs a **Plan Understanding Quiz** -- a brief pre-flight check that verifies you genuinely understand the plan you are about to execute.
...
If you answer both correctly, the loop proceeds immediately. If you miss one or both, Loop explains what the plan actually does and offers a choice: proceed anyway, or stop and review.
...
For an input plan named `plan.md`, the default QA output path is `.loop/plan_qa/plan-qa.md`.
...
Responses are saved to `.loop/skill/<timestamp>/` with `input.md`, `output.md`,
and `metadata.md` for reference.

## Configuration

Loop uses a 4-layer config hierarchy (lowest to highest priority):
1. **Plugin defaults**: `config/default_config.json`
2. **User config**: `~/.config/loop/config.json`
3. **Project config**: `.loop/config.json`
...
To override, add to `.loop/config.json`:
...
On Codex installs, Loop also seeds `${XDG_CONFIG_HOME:-~/.config}/loop/config.json`
...
**Migration note**: If your `.loop/config.json` contains the legacy keys
...
Progress data is stored in `.loop/rlcr/<timestamp>/` for each loop session.
...
export LOOP_CODEX_BYPASS_SANDBOX=true

# Or set for a single session
LOOP_CODEX_BYPASS_SANDBOX=true claude --plugin-dir /path/to/loop
```
