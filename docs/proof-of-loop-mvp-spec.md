...
Both of Loop's scans call the Proof layer's own marker grammar rather than repeating it, so a gate cannot certify as absent a finding the Proof layer would have read — the two agree by construction, not by matching test cases. Where the detector cannot establish a clean review it records nothing, and drops any earlier all-clear for that round. A scan that could not run counts as not established: "could not look" is never "looked and saw nothing". Which record is an all-clear is decided structurally, by the absence of any marker: a findings record is copied verbatim from review output and begins at the marker line that triggered extraction, so review output cannot make itself look like an all-clear and be deleted.
