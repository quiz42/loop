...
- Items exceeding `max_item_bytes` are written as `status: "truncated"`: a summary plus the original `sha256` and original byte count. The Validator only checks the declaration's self-consistency and records `truncated-evidence`. Self-consistency here means four things, all derivable from the Bundle alone: the item carries `omitted_reason: "size-limit"`; its declared `bytes` exceeds the `max_item_bytes` of the profile the Bundle pins; no filesystem entry exists at its declared `evidence/<path>`, exactly as for an omitted item; and its `summary` describes an artifact that size could have. `proof.json` defines the verified Evidence Item set. Undeclared files co-located under `evidence/` are unmanaged transport content: they are not Evidence Items, are not hashed or counted toward `max_bundle_bytes`, and receive no integrity, disclosure, or secret-safety claim.
- **The summary carries no content**, and that is a decision rather than an omission ([ADR-0008](adr/0008-truncation-is-a-disclosure-boundary-so-the-summary-carries-no-content.md)). Truncation is a disclosure boundary, not only a size accident: an item withheld for size is unpublished, and section G's projections are stripped across that line — the goal when `plan.md` is unpublished, the acceptance criteria when the Goal Tracker is, the recorded timestamps and `loop_version` when the state file is, and review prose behind a finding hash. A summary quoting the withheld bytes would hand back precisely what those rules remove. It carries shape instead: `{"algo": "evidence-summary-v0", "lines": N, "longest_line_bytes": M}`, counted on raw bytes with no decoding and no line-ending normalization, and constrained by `lines <= bytes`, `longest_line_bytes <= bytes`, `longest_line_bytes + max(lines - 1, 0) <= bytes` and `bytes <= lines * (longest_line_bytes + 1)`. A summary on any other status is a profile violation.
...
Both of Loop's scans call the Proof layer's own marker grammar rather than repeating it, so a gate cannot certify as absent a finding the Proof layer would have read — the two agree by construction, not by matching test cases. The findings tail is likewise cut out of the very text the grammar scanned, never re-located through a line number handed to a second reader that counts lines differently. Where the detector cannot establish a clean review it records nothing. A scan that could not run counts as not established: "could not look" is never "looked and saw nothing".

No record is published without a re-scan of the published bytes. Every record is written to a temporary file, re-read through the same grammar, and atomically renamed into place only if it still reads as what it claims — a findings record as carrying an actionable marker, an all-clear as carrying none. A record that fails its own re-scan is withheld, so a writer/reader disagreement of any future shape costs the round its record rather than producing a false one.

Each detection attempt that reaches its review log drops the round's previous all-clear before scanning it. An all-clear is an assertion the new attempt has yet to earn, not evidence, and it must not survive an attempt that could not re-establish it — least of all one whose scanner could not run. An attempt with no log to read blocks for retry without touching the record: it learned nothing, and the last completed review's earned record outranks a retry that never looked. If raw-byte classification of an existing record fails, Loop preserves the record as evidence and blocks the attempt; an unclassifiable record can never authorize finalization. Which record is an all-clear is decided structurally, over raw bytes and without the scanner: a findings record is copied verbatim from review output and begins at the marker line that triggered extraction, so it always carries `[P`; the all-clear never does, and review output cannot make itself look like one and be deleted. When the scanner is unavailable, the ASCII fallback still surfaces findings to the loop but publishes no record at all: publishing is an assertion, and the grammar that must verify assertions is exactly what is missing.
...
Masking exists because omission alone turned out to be too blunt for one kind. `codex review` cites the file it faults by absolute path, so the path rule withheld precisely the review results that reported findings — see [`proof-of-loop-m4-dogfood.md`](proof-of-loop-m4-dogfood.md) and ADR-0004. The revision ships as a new profile name, `public-v1`, because a profile document is immutable once a Bundle has pinned its hash (ADR-0006); `public-v0` stays frozen so Bundles that pinned it keep verifying. `public-v1` sets `mask_on: ["absolute-path"]` and `mask_kinds: ["round_review_result"]`, keeping `omit_on` as the fallback for anything masking cannot clean (bytes that are not UTF-8, an oversized source, a substitution that would leave a match behind, or a substitution that no longer fits `max_item_bytes`). That fallback is not optional: the placeholder is longer than most paths it replaces, so a source within the cap can substitute to over it, and an item the profile can neither mask nor omit would be published with the paths masking exists to remove. A profile naming a scan class in `mask_on` must therefore name it in `omit_on` as well. Profiles load by name from the shipped profile directory, so this is a property of the documents in this repository rather than of untrusted input, and `tests/proof_contract/test_contract.py` holds all three of them to it.
