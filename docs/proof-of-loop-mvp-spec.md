...
Both of Loop's scans read a marker exactly where the Proof layer does — the token starts within the first ten columns of the line — so a gate can never certify as absent a finding the Proof layer would have read. Where the detector cannot establish a clean review it records nothing, and drops any earlier all-clear for that round. Which record is an all-clear is decided structurally, by the absence of any marker: a findings record is copied verbatim from review output and begins at the marker line that triggered extraction, so review output cannot make itself look like an all-clear and be deleted.
