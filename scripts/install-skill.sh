#!/usr/bin/env bash
#
# Install/upgrade Loop skills for Kimi and/or Codex.
#
# What this does:
# 1) Sync skills/{loop,loop-gen-plan,loop-rlcr} to target skills dir(s)
# 2) Copy runtime dependencies into <skills-dir>/loop/{scripts,hooks,prompt-template}
...
#
# Options:
#   --repo-root PATH        Loop repo root (default: auto-detect)
...
LOOP_USER_CONFIG_DIR="${XDG_CONFIG_HOME:-${HOME}/.config}/loop"
COMMAND_BIN_DIR="${LOOP_COMMAND_BIN_DIR:-${HOME}/.local/bin}"
LEGACY_SKILLS_DIR=""
DRY_RUN="false"
...
usage() {
    cat <<'EOF'
Install Loop skills for Kimi and/or Codex.
...
resolve_source_layout() {
    ...
    #   <skills-dir>/loop/scripts/install-skill.sh
    #   <skills-dir>/loop-gen-plan/SKILL.md
    #   <skills-dir>/loop-rlcr/SKILL.md
    if [[ -d "$runtime_root/scripts" ]] && [[ -d "$runtime_root/hooks" ]] && [[ -d "$runtime_root/prompt-template" ]]; then
        skills_root="$(cd "$runtime_root/.." && pwd)"
        if [[ -f "$skills_root/loop/SKILL.md" ]] && [[ -f "$skills_root/loop-gen-plan/SKILL.md" ]] && [[ -f "$skills_root/loop-refine-plan/SKILL.md" ]] && [[ -f "$skills_root/loop-rlcr/SKILL.md" ]]; then
    ...
    die "could not resolve Loop source layout from: $candidate_root"
}
...
install_runtime_bundle() {
    local target_dir="$1"
    local runtime_root="$target_dir/loop"
    local component
...
hydrate_skill_runtime_root() {
    ...
    for skill in "${SKILL_NAMES[@]}"; do
        ...
        _HYDRATE_RUNTIME_ROOT="$runtime_root" \
            awk '{gsub(/\{\{LOOP_RUNTIME_ROOT\}\}/, ENVIRON["_HYDRATE_RUNTIME_ROOT"]); print}' "$skill_file" > "$tmp" \
...
install_codex_user_config() {
    ...
    local user_config_dir="${LOOP_USER_CONFIG_DIR}"
    ...
    if ! command -v python3 >/dev/null 2>&1; then
        die "python3 is required to update Loop user config for Codex installs"
    fi
...
PY
    case "$?" in
        ...
        2)
            ...
            ;;
        *)
            die "failed to update Loop user config at $user_config_file"
            ;;
    esac
}
...
echo "Error: Loop bitlesson selector runtime not found. Re-run install-skill.sh." >&2
exit 1
EOF
...
install_codex_target() {
    ...
    install_codex_user_config "$CODEX_SKILLS_DIR/loop" "$TARGET"
...
case "$TARGET" in
    kimi)
        install_kimi_target
        install_bitlesson_selector_shim "$KIMI_SKILLS_DIR/loop"
        ;;
    codex)
        install_codex_target
        install_bitlesson_selector_shim "$CODEX_SKILLS_DIR/loop" "$KIMI_SKILLS_DIR/loop"
        ;;
    both)
        install_kimi_target
        install_codex_target
