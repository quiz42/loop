#!/usr/bin/env bash
#
# monitor-skill.sh - Skill monitor for loop
#
# Provides the _loop_monitor_skill function for monitoring
# skill invocations (ask-codex, ask-gemini) from .loop/skill directory.
#
# This file is sourced by loop.sh and depends on:
# - monitor-common.sh (monitor_get_yaml_value, monitor_format_timestamp, etc.)
# - loop.sh (loop_split_to_array)

# Monitor skill invocations from .loop/skill
...
_loop_monitor_skill() {
    ...
    # Returns the ~/.cache/loop/... path if it exists, empty otherwise.
    _skill_find_cache_dir() {
        ...
        local cache_dir="$cache_base/loop/$sanitized/skill-$unique_id"
        [[ -d "$cache_dir" ]] && echo "$cache_dir" || echo ""
    }

    # Find the best file to monitor for a skill invocation
    # Searches both global cache (~/.cache/loop/), local cache ($dir/cache/),
    # and project-local files (.loop/skill/) for the best content.
    ...
    _skill_monitor_title() {
        case "$tool_filter" in
            codex)  echo " Loop Skill Monitor [codex]" ;;
            gemini) echo " Loop Skill Monitor [gemini]" ;;
            *)      echo " Loop Skill Monitor" ;;
        esac
    }
    ...
    _skill_draw_status_bar() {
        ...
        # Aggregate stats
        local -a stats
        loop_split_to_array stats "$(_skill_count_stats)"
