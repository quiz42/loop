...
load_merged_config() {
    ...
    if [[ -n "${XDG_CONFIG_HOME:-}" ]]; then
        user_config_path="$XDG_CONFIG_HOME/loop/config.json"
    else
        user_config_path="${HOME:-}/.config/loop/config.json"
    fi

    if [[ -n "${LOOP_CONFIG:-}" ]]; then
        project_config_path="$LOOP_CONFIG"
    else
        project_config_path="$project_root/.loop/config.json"
    fi
