...
MONITORING:
  # View current state:
  cat .loop/rlcr/*/state.md

  # View latest summary:
  cat .loop/rlcr/*/round-*-summary.md | tail -50

  # View Codex review:
  cat .loop/rlcr/*/round-*-review-result.md | tail -50
HELP_EOF
    exit 0
}
...
RLCR_LOOP_DIR=$(find_active_loop "$PROJECT_ROOT/.loop/rlcr" 2>/dev/null || echo "")
...
if [[ -z "$PLAN_FILE" ]]; then
    if [[ "$SKIP_IMPL" == "true" ]]; then
        ...
        PLAN_FILE=".loop/skip-impl-placeholder.md"
...
# Filter out untracked .loop/ paths and .loop-* dash-separated legacy variants.
# These are gitignored runtime directories and do not indicate a dirty working tree.
GIT_STATUS_OUTPUT=$(echo "$GIT_STATUS_OUTPUT" | grep -vE '^\?\? \.loop[-/]' || true)
...
SKIP_IMPL_PLAN_EOF
    ...
    PLAN_FILE=".loop/rlcr/$TIMESTAMP/plan.md"
else
...
printf '%s\n%s\n' "$LOOP_DIR/state.md" "$SCRIPT_SELF_PATH" > "$PROJECT_ROOT/.loop/.pending-session-id"
