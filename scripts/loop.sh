#!/usr/bin/env bash
# loop.sh - Loop shell utilities
# Part of rc.d configuration
# Compatible with both bash and zsh

# Source shared monitor utilities (per plan: scripts/lib/monitor-common.sh)
LOOP_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
if [[ -f "$LOOP_SCRIPT_DIR/lib/monitor-common.sh" ]]; then
    source "$LOOP_SCRIPT_DIR/lib/monitor-common.sh"
fi

# Source shared loop library (provides DEFAULT_CODEX_MODEL and other constants)
LOOP_HOOKS_LIB_DIR="$(cd "$LOOP_SCRIPT_DIR/../hooks/lib" && pwd)"
if [[ -f "$LOOP_HOOKS_LIB_DIR/loop-common.sh" ]]; then
    source "$LOOP_HOOKS_LIB_DIR/loop-common.sh"
fi
...
# Usage: loop_split_to_array "output_array_name" "value1|value2|value3"
loop_split_to_array() {
    local arr_name="$1"
    local input="$2"
...
loop_parse_goal_tracker_issue_counts() {
...
loop_parse_goal_tracker() {
    ...
    local -a issue_parts
    loop_split_to_array issue_parts "$(loop_parse_goal_tracker_issue_counts "$tracker_file")"
...
# Monitor the latest Codex run log from .loop/rlcr
...
_loop_monitor_codex() {
    ...
    # Log files are now in $HOME/.cache/loop/<sanitized-project-path>/<timestamp>/ to avoid context pollution
    ...
    _find_latest_codex_log() {
        ...
        local cache_base="${XDG_CACHE_HOME:-$HOME/.cache}/loop"
    ...
    # These maintain backward compatibility within _loop_monitor_codex
    _parse_goal_tracker() { loop_parse_goal_tracker "$@"; }
    _parse_git_status() { loop_parse_git_status "$@"; }
    _split_to_array() { loop_split_to_array "$@"; }

    # Draw the status bar at the top
    _draw_status_bar() {
        # Note: ksharrays is set at _loop_monitor_codex() level for zsh compatibility
        ...
        local -a issue_parts
        _split_to_array issue_parts "$(loop_parse_goal_tracker_issue_counts "$goal_tracker_file")"
    ...
    _display_terminal_too_small() {
        ...
        local message="This Loop Monitor requires at least $min_height lines to work"
    ...
    while [[ "$monitor_running" == "true" ]]; do
        ...
        if [[ ! -d "$loop_dir" ]]; then
            _graceful_stop ".loop/rlcr directory no longer exists"
            return 0
        fi
        ...
        monitor)
            ...
            case "$target" in
                ...
                skill)
                    ...
                    ;;
                codex)
                    _loop_monitor_skill --tool-filter codex "$@"
                    ;;
                gemini)
                    _loop_monitor_skill --tool-filter gemini "$@"
                    ;;
                *)
                    echo "Usage: loop monitor <rlcr|skill|codex|gemini>"
                    echo ""
                    echo "Subcommands:"
                    echo "  rlcr    Monitor the latest RLCR loop log from .loop/rlcr"
...
# Source skill monitor (provides _loop_monitor_skill)
if [[ -f "$LOOP_SCRIPT_DIR/lib/monitor-skill.sh" ]]; then
    source "$LOOP_SCRIPT_DIR/lib/monitor-skill.sh"
fi
