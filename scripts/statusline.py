...
def render_status_line(cwd: str | Path = ".", width: int | None = None, env: dict[str, str] | None = None) -> str:
    ...
    model = values.get("LOOP_MODEL") or values.get("CODEX_MODEL") or values.get("ANTHROPIC_MODEL") or "model unset"
    loop_status = values.get("LOOP_STATUS") or "idle"
...
def render_from_json(payload: str, width: int | None = None) -> str:
    ...
    env = dict(os.environ)
    for key in ("LOOP_MODEL", "CODEX_MODEL", "ANTHROPIC_MODEL", "LOOP_STATUS"):
        if key in data:
