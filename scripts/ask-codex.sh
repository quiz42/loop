...
#
# Storage:
#   Project-local: .loop/skill/<unique-id>/{input,output,metadata}.md
#   Cache: ~/.cache/loop/<sanitized-path>/skill-<unique-id>/codex-run.{cmd,out,log}
#
...
EXAMPLES:
  /rloop:ask-codex How should I structure the authentication module?
  /rloop:ask-codex --codex-model gpt-5.5:high What are the performance bottlenecks?
  /rloop:ask-codex --codex-timeout 300 Review the error handling in src/api/
...
if ! command -v codex &>/dev/null; then
    ...
    echo "Then retry: /rloop:ask-codex <your question>" >&2
    exit 1
fi
...
if [[ -z "$QUESTION" ]]; then
    ...
    echo "" >&2
    echo "Usage: /rloop:ask-codex [OPTIONS] <question or task>" >&2
    echo "" >&2
    echo "For help: /rloop:ask-codex --help" >&2
    exit 1
fi
...
# Project-local storage: .loop/skill/<unique-id>/
SKILL_DIR="$PROJECT_ROOT/.loop/skill/$UNIQUE_ID"
mkdir -p "$SKILL_DIR"

# Cache storage: ~/.cache/loop/<sanitized-path>/skill-<unique-id>/
# Falls back to project-local .loop/cache/ if home cache is not writable
SANITIZED_PROJECT_PATH=$(echo "$PROJECT_ROOT" | sed 's/[^a-zA-Z0-9._-]/-/g' | sed 's/--*/-/g')
CACHE_BASE="${XDG_CACHE_HOME:-$HOME/.cache}"
CACHE_DIR="$CACHE_BASE/loop/$SANITIZED_PROJECT_PATH/skill-$UNIQUE_ID"
...
if [[ $CODEX_EXIT_CODE -eq 124 ]]; then
    ...
    echo "  /rloop:ask-codex --codex-timeout $((CODEX_TIMEOUT * 2)) <your question>" >&2
    echo "" >&2
