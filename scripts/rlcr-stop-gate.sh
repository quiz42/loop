...
source "$LOOP_ROOT/hooks/lib/project-root.sh"
PROJECT_ROOT="$(resolve_project_root 2>/dev/null || true)"
HOOK_SCRIPT="$LOOP_ROOT/hooks/loop-codex-stop-hook.sh"

SESSION_ID="${CLAUDE_SESSION_ID:-}"
TRANSCRIPT_PATH="${CLAUDE_TRANSCRIPT_PATH:-}"
PRINT_JSON="false"
HOOK_MODEL="${CODEX_MODEL:-loop-skill-gate}"
...
if [[ -z "$PROJECT_ROOT" ]]; then
    # No loop project context reachable from here -- nothing to enforce.
    ...
    echo "ALLOW: no loop project root resolved."
    exit 0
fi
