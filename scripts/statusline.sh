...
# an RLCR status field tied to the Loop plugin. It is included in
...
# is generic session information. Future updates to the Loop plugin
...
# Mirrors find_active_loop() logic from loop hooks/lib/loop-common.sh
get_rlcr_status() {
    local rlcr_dir="$1"
...
if [[ -n "$CWD" && -d "$CWD/.loop" ]]; then
    RLCR_STATUS=$(get_rlcr_status "$CWD/.loop/rlcr" "$SESSION_ID")
else
    RLCR_STATUS="Off"
fi
