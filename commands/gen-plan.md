# Command: /humanize:gen-plan

Generate a structured implementation plan from an idea document. The output `PLAN.md` is the primary input for `/humanize:start-rlcr-loop` and drives all agent activity during the loop.
...
```
/humanize:gen-plan [options]
```

Internally runs:

```
humanize gen-plan [--input IDEA.md] [--output PLAN.md] [--title TITLE]
```
...
|--------|---------|-------------|
| `--input FILE` | `IDEA.md` | Path to the idea document produced by `/humanize:gen-idea` |
...
The generated `PLAN.md` is consumed directly by `/humanize:start-rlcr-loop`.
...
/humanize:gen-plan --input ideas/auth-refactor.md --output plans/auth-plan.md

# Override the title
/humanize:gen-plan --input IDEA.md --output PLAN.md --title "Dark Mode Implementation Plan"
```

## Expected Output

```
[humanize] Reading idea from IDEA.md...
[humanize] Generating implementation plan...
[humanize] Plan written to PLAN.md (7 goals, 12 tasks, 5 acceptance criteria)
```
