...
3. `--qa-dir <path>` is optional. If omitted, set `QA_DIR=.loop/plan_qa`.
...
2. Optional user config: `${XDG_CONFIG_HOME:-$HOME/.config}/loop/config.json`
3. Optional project config: `${LOOP_CONFIG:-$PROJECT_ROOT/.loop/config.json}`
