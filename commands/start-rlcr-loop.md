...
2. Use the Task tool to invoke the `loop:plan-compliance-checker` agent (sonnet model):
   ```
...
2. Use the Task tool to invoke the `loop:plan-understanding-quiz` agent (opus model):
   ```
   Task tool parameters:
   - model: "opus"
   - prompt: Include the plan file content and ask the agent to:
...
If a problem is solved only after multiple rounds, add or update a precise lesson entry in `.loop/bitlesson.md` (specific problem + specific solution).
By default, empty `.loop/bitlesson.md` does not block `Action: none`; use `--require-bitlesson-entry-for-none` to enforce strict blocking.
